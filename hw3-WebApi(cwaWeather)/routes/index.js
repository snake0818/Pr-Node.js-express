...
// 全域變數'lock'用以紀錄目前操控按鈕是否已被鎖住：若lock==0，代表操控按鈕沒有鎖住;若lock==1，代表操控按鈕已被鎖住，
...
//*********************** 使用中央氣象局開放天氣Web API查詢天氣狀況之服務方法******************
//******* 客戶端提出 POST /cwbweather/ 請求時，執行此服務方法(匿名式函數) ********
router.post('/weather', (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*'); //  允許其他網站的網頁存取此服務

  // create the url for querying cwb weather API
  let baseurl = "https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-C0032-001?";
  let AuthCode = "CWB-F92136A9-0AA3-4E52-8C2B-95B598005F6E";
  let cwbweatherurl = `${baseurl}`;
  //console.log("URL: " + cwbweatherurl);
    ...
    function (err, response, body) {
      if (err) {
        ...
        try {
          ...
          // 添加進資料時間
          cities.push({
            'Day': sdata.records.location[0].weatherElement[0].time[0].startTime.split(" ")[0].split("-")[1]+'/'+sdata.records.location[0].weatherElement[0].time[0].startTime.split(" ")[0].split("-")[2],
            'StartTime': sdata.records.location[0].weatherElement[0].time[0].startTime.split(" ")[1].split(":")[0]+':00',
            't1': sdata.records.location[0].weatherElement[0].time[1].startTime.split(" ")[1].split(":")[0]+':00',
            't2': sdata.records.location[0].weatherElement[0].time[2].startTime.split(" ")[1].split(":")[0]+':00',
            'EndTime': sdata.records.location[0].weatherElement[0].time[2].endTime.split(" ")[1].split(":")[0]+':00',
          })
          // 總共22個縣市
          for (i = 0; i < 22; i++) {
            // Wx天氣狀況、PoP降雨機率、CI舒適度指數、MinT最低溫度、MaxT最高溫度
            var Wx = []
            var PoP = []
            var CI = []
            var MinT = []
            var MaxT = []
            city = sdata.records.location[i].locationName;
            for (j = 0; j < 3; j++) {
              Wx.push(sdata.records.location[i].weatherElement[0].time[j].parameter)
              PoP.push(sdata.records.location[i].weatherElement[1].time[j].parameter)
              CI.push(sdata.records.location[i].weatherElement[3].time[j].parameter)
              MinT.push(sdata.records.location[i].weatherElement[2].time[j].parameter)
              MaxT.push(sdata.records.location[i].weatherElement[4].time[j].parameter)
            }
            // 建立城市簡單天氣資訊之JSON物件
            cities.push({ 'City': city, 'Wx': Wx, 'PoP': PoP, 'CI': CI, 'MinT': MinT, 'MaxT': MaxT })
          }
        } catch (ex) {
          console.log("解析天氣資料時發生例外，原因如下: " + ex);
          cities = {};
      ...
      res.send(cities);	// 將城市之天氣資料回傳給Client端	
    }
  );
})
